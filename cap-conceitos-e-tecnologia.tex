...
conceitual realizada neste trabalho é descrita no Capítulo
\ref{cap:projeto_tecnico}.
...
\section{SQL e álgebra relacional}
\label{sec:sql_e_algebra_relacional}
...
\label{sec:orientacao_a_objetos}
...
\captionof{lstlisting}{Exemplo de como podemos criar 3 classes e adicionar seus
atributos}
...
\end{lstlisting}
\captionof{lstlisting}{Exemplo de utilização do encapsulamento para calcular um
bonus}
...
\end{lstlisting}
\captionof{lstlisting}{Exemplo de uma classe que representa um usuário no
sistema}
...
\end{lstlisting}
\captionof{lstlisting}{Exemplo de como criamos classes filhas utilizando a
herança do Java}
...
\end{lstlisting}
\captionof{lstlisting}{Exemplo de polimorfismo da classe filha Professor e
classe mãe Usuario}

Note que foi utilizada uma \textit{annotation} do Java que indica que o método
...
\label{sec:mapeamento_objeto_relacional}
...
terão. A aplicação desse conceito no Java, é descrita com mais na seção \ref{JPA
...
\section{Dispositivos Móveis e Aplicações}
\label{sec:dispositivos_moveis_e_aplicacoes}

% pesquisar depois sobre o uso de dispositivos móveis
...
item basta escrever 3 ou 4 linhas.
...
Esse método precisa ser escrito de uma forma bem explícita sobre o que está sendo testado, 
para esse caso poderíamos escrever o seguinte método:
...
\label{subsec:instalacao_via_dpkg}
...
\label{subsec:instalacao_do_git}
