...
precisa utilizando o JUnit. Esse método precisa ser escrito de uma forma bem
explícita sobre o que está sendo testado, para esse caso poderíamos escrever o
seguinte método:
...
de resolver diversos problemas em que são comuns em uma aplicação web, na aplicação foi
...
objetos, sempre que queremos utilizar uma classe, precisamos fazer um instância,
no caso de APIs, além de fazer uma instância, na maioria das vezes,
...
injenção de dependência, que consiste em, ensinar o Spring a instânciar as
nossas classes, mesmo que elas exigam uma configuração apropriada para o
funcionamento, e então fazer com que ele se responsabilize por gerenciar todo
tipo de instância que ensinarmos, ou seja, inverter o controle da criação dos
objetos da aplicação. 

\subsection{controle de transações}
\label{subsec:controle_de_transações}

Para conenctar a aplicação com o banco de dados, foi utilizada a especificação
