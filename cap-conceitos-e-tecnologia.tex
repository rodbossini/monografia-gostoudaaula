...
o modelo MVC consiste em dividir a aplicação em camadas:
\begin{itemize}
...
tanto para a equipe de desenvolvimento quanto para o cliente. 
...
estiver ok, ele responde com as informações do aluno/professor junto com o um
token identificador. O diagrama de sequência não é utilizado em todas as
...
realizar algumas alterações em sua estrutura, como por exemplo a adição ou
alteração ou, em casos mais delicados, a exclusão de uma coluna ou tabela. Tais
...
alteração é arriscada, pois, em uma instância do seu projeto tudo estava
funcionando sem algum problema, porém, devido a um ajuste, possíveis falhas
podem ocorrer, como por exemplo, uma funcionalidade que funcionava e, de
repente, para de funcionar.\\

Para evitar esse tipo de problema, utilizamos ferramentas de versionamento, que
permitem a realização de um backup de uma instância do projeto, e então, podemos
realizar diversas alterações sem nos preocupar se o ajuste será problemático ou
não para o sistema. Para esse projeto, utilizamos a ferramenta
Git~\ref{subsec:git}
...
Jackson entre outros. Essas APIs são arquivos .jar que precisam ser
baixados e adicionados dentro do projeto, porém, quando fazemos isso
...
ele necessita da API da JPA. O Maven resolve todos esses problemas simplesmente
adicionando a dependência por meio da tag <denpendency>: 
...
máximo possível de \textit{bugs}, ou seja, tomar alguns cuidadoa com qual tipo
de configuração a nossa aplicação está utilizando, pois elas são diferentes, ou
seja, consequentemente teremos um erro. \\
...
framework~\cite{subsec:spring_framework}. O objetivo desse módulo é justamente
...
o flyway~\cite{flyway} que é uma farramenta simples e poderosa, capaz de
...
O Maven~\cite{gradle} é uma biblioteca capaz de gerenciar projetos Java, com a
