...
pesquisa padrão para banco de dados relacionais que permite a comunicação com
um banco de dados para a realização de consultas simples ou com álgebra
relacional. A álgebra relacional é uma teoria que representa,
matematicamente, a forma como podemos manipular as consultas realizadas
utilizando o SQL, como por exemplo, mencionar quais são os atributos e filtros necessários com o intuito
de devolver o resultado esperado. Veja como exemplo a
Equação~\ref{eq:algebra_relacional}. Nela, o operador~$\sigma$ atua sobre a
relação Empregado, aplicando o filtro~$id = 1$, o que quer dizer que somente as
tuplas com essa propriedade fazem parte da relação obtida.

\begin{equation}
	\sigma_{id = 1} (Empregado)
	\label{eq:algebra_relacional}
\end{equation}
...
desenvolvimento de software em que utilizamos unidades chamadas de
...
objeto da vida real para a aplicação. Na modelagem
da aplicação proposta neste trabalho, por exemplo, foi necessário modelar a
entidade aula que representa uma aula da vida real que compõe um aluno, um professor e qualquer outra informação que seja necessária
para essa entidade.
...
na aplicação proposta, foi realizada a abstração de um usuário qualquer no
sistema, o resultado obtido dessa abstração foi a classe \lstinline{Usuario} conforme o
...
computacional, porém, uma das soluções possíveis e sofisticadas dentro da
orientação a objetos, é fazer com que as classes \lstinline{Professor} e
\lstinline{Aluno} herdem da classe \lstinline{Usuario}, pois, dessa forma, faremos com que ambas possuam os mesmo comportamentos de um
...
instrução \lstinline{extends}, exemplificada pelo 
...
reescrevemos o método da classe \lstinline{Usuario} (classe mãe) dentro da classe \lstinline{Professor} (classe filha) e adicionamos o
 comportamento específico e esperado daquele método para a classe \lstinline{Professor}, veja o exemplo a partir do
...
terão. A aplicação desse conceito no Java, é descrita com mais detalhes na Seção
~\ref{subsec:jpa_java_persistence_api}.

\subsection{Web Services}
\label{subsec:web_services}

Um web service é uma solução muito utilizada no mercado para permitir que
...
Representacional, é uma abstração arquitetural para comunicação na Web
frequentemente utilizada em web services~\ref{subsec:web_services}. Uma de suas
aplicações dentro de um web service é a distribuição de APIs. Essa arquitetura
...
que é o sistema operacional do Apple iPhone. O
gráfico~\ref{fig:grafico_android_2012} demonstra a participação no mercado dos
...
entre iOS e Windows Phone 7. A Figura ~\ref{fig:grafico_android_2016} ilustra a
...
ganhar terreno tentando repetir o sucesso do Amazon Kindle Fire, que também
utiliza Android.
...
representar um sistema computacional completo em 3 camadas distintas. Abaixo,
estão listadas tais camadas e seus objetivos respectivamente.
...
A Figura~\ref{fig:mvc}, ilustra graficamente esta arquitetura.
...
precisa antes de criar a funcionalidade de fato.\\
...
Observe que a ideia de testes unitários e de alta coesão aparece intrinsicamente
neste cenário.\\
...
Além de ser trabalhoso, provavelmente, esqueceremos de testar todas as funcionalidades do sistema, ou seja, a chance de não garantir que 
tudo esteja funcionando como o esperado, é muito grande. Quando utilizamos o TDD, todos os cenários de testes que criamos
...
diversos usuários (clientes) e responde de acordo com o que cada cliente
...
A UML - Unified Modeling Language, na tradução, Linguagem Unificada de
...
tanto para a equipe de desenvolvimento quanto para o cliente. Na UML, tais
...
No diagrama de caso de uso podemos demonstrar as principais
funcionalidades e seus atores dentro da aplicação, por exemplo, dentro da
...
e que os atores são o aluno e o professor. Esta modelagem revela somente as
funcionalidades de um ponto de vista muito superficial. Nenhum detalhe técnico é
exibido aqui.
...
atores \textbf{aluno} ou \textbf{professor}, ambos enviam suas credenciais para
...
funcionalidades, em outras palavras, a intenção do diagrama de sequência é
...
desenvolvimento do banco de dados antes mesmo de realizar a
implantação~\ref{subsubsec:distribuicao_da_aplicacao_deploy}, pois se uma versão
for diferente, o desenvolvedor que realizar a implantação com a versão diferente
pode causar diversos problemas inesperados em um ambiente de produção.\\
...
resultado inesperado, como por exemplo, uma funcionalidade que operava
...
que proporcionam esse sistema de versionamento. Para esse projeto,
...
Esta seção especifica todas as tecnologias que foram utilizadas para o
...
Essas APIs são arquivos .jar que precisam ser baixados e adicionados ao projeto,
porém, quando fazemos isso manualmente, nem sempre é uma tarefa simples e tende a apresentar problemas,
como por exemplo o Hibernate que, além de precisar das bibliotecas próprias,
 necessita da API da JPA~\ref{subsec:jpa_java_persistence_api}. O Maven
resolve todos esses problemas simplesmente adicionando a dependência por meio da tag \lstinline{<dependency>} onde é
...
deploy da aplicação nem sempre é uma tarefa trivial, pois é necessário se preocupar com
ambientes diferentes, por exemplo, em ambiente de desenvolvimento,
...
outras palavras, o projeto foi dividido em módulos. Cada módulo representa um
projeto distinto que tem apenas uma única finalidade, por exemplo, para
...
verifica se a funcionalidade de cadastrar um aluno está funcionando é exibido no
Código~\ref{lst:junit}.


\begin{lstlisting}[caption={Exemplo de um caso de teste utilizando o JUnit},
label = {lst:junit}] @Test
...
momento em que executar o JUnit, ele fará parte da pilha de testes. Porém, para
...
ver um exemplo a partir do Código~\ref{lst:asserts_junit}.
...
O Spring framework é um conjunto de módulos (projetos)~\cite{spring} que
tem a finalidade de resolver diversos problemas que são comuns em uma aplicação
web. Na aplicação desenvolvidaa, ele foi utilizado para resolver as questões de
controle de transações e injeção de dependência.
...
externas, é bem comum que, além de fazer uma instância da classe fornecida, haja
...
desejado, por padrão, o Spring não sabe como instanciar os objetos. Em outras
...
ensine o Spring a instanciar tal objeto.\\

A partir do momento em que o Spring sabe instanciar um objeto, e então, pedimos
...
controle}, ou, em inglês, \textit{Inversion of Control} (IoC).\\
...
classes que o Spring sabe instanciar.
Para transformarmos uma interface ou classe em um bean, basta adicionar uma
tag \lstinline{<bean>} no arquivo XML de configuração do spring, conforme o
...
Spring, temos a possibilidade de utilizar todo o conceito de injeção de
...
Perceba que essas ações podem ser um pouco repetitivas como também perigosas
para a nossa aplicação, como por exemplo, interagir com o banco por meio de uma transação e não
...
forma mais organizada e com menos intervenção humana, nesse projeto, utilizamos
...
services~\ref{subsec:web_services}, é muito comum convertermos classes para um
...
de uma classe para o formato desejado nem sempre é um trabalho fácil e por isso
não é recomendável esse tipo de prática. Para facilitarmos essa tarefa, utilizamos a biblioteca do Jackson que, com poucas
...
tenha um desempenho melhor. Nesse projeto não serão detalhadas informações sobre
...
diversos fins, porém, para essa aplicação, foi utilizada a versão Java EE
